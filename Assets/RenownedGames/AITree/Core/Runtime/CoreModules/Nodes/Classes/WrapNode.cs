...
namespace RenownedGames.AITree
{
    public abstract class WrapNode : Node
    {
        ...
        {
            WrapNode wrap = base.Clone() as WrapNode;
            wrap.decorators = decorators.ConvertAll(d => d.Clone() as DecoratorNode);
            wrap.services = services.ConvertAll(s => s.Clone() as ServiceNode);
            return wrap;
        }
        ...
        private void OnNotified(ObserverDecorator observer, bool result)
        {
            ...
            if (GetOrder() <= currentOrder)
            {
                ...
                bool isSubNode = BehaviourTree.IsSubNode(this, GetBehaviourTree().GetCurrentNode());
